...
public class TextSticker extends Sticker {
    ...
    public TextSticker setTextColor(@NonNull ColorModel color) {
        if (color.getColorStart() == color.getColorEnd()) {
            ...
            Shader shader = new LinearGradient(setDirection(color.getDirec())[0],
                    ...
                    new int[]{Color.parseColor(UtilsBitmap.INSTANCE.toRGBString(color.getColorStart())), Color.parseColor(UtilsBitmap.INSTANCE.toRGBString(color.getColorEnd()))},
    ...
    public TextSticker setShadow(float radiusBlur, float dx, float dy, int color) {
        ...
        if (color != 0)
            textPaint.setShadowLayer(radiusBlur, dx, dy, Color.parseColor(UtilsBitmap.INSTANCE.toRGBString(color)));
