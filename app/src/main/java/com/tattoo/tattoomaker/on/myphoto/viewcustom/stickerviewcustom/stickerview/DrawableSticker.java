...
public class DrawableSticker extends Sticker {
    ...
    public void draw(@NonNull Canvas canvas) {
        ...
        if (isShadow)
            UtilsBitmap.INSTANCE.drawIconWithPath(canvas, path, paintShadow, realBounds.width(), 0, 0);

        UtilsBitmap.INSTANCE.drawIconWithPath(canvas, path, paint, realBounds.width(), 0, 0);
    ...
    public void setShadow(float radiusBlur, float dx, float dy, int color, boolean isDecorOrTemp) {
        ...
        if (color == 0) paintShadow.setShadowLayer(radiusBlur, dx, dy, Color.BLACK);
            ...
            paintShadow.setShadowLayer(radiusBlur, dx, dy, Color.parseColor(UtilsBitmap.INSTANCE.toRGBString(color)));
    }
