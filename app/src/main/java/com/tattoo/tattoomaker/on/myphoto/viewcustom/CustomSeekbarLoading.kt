...
class CustomSeekbarLoading(context: Context) : View(context) {
    ...
    init {
        ...
        paint_Text = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            ...
            typeface = Utils.getTypeFace("solway", "solway_regular.ttf", context)
        }
        paintStroke = Paint(Paint.ANTI_ALIAS_FLAG).apply { style = Paint.Style.STROKE }
        rectF = RectF()
        rectF2 = RectF()
        progress = 100
        padding = 0.84f * w / 100f
    ...
    fun setColorPro(colorPro1: Int, colorPro2: Int) {
        if (colorPro1 == colorPro2) paintPr.color = colorPro1
        else {
            paintPr.shader = LinearGradient(
                0f, 0F, 0F,6.11f * w / 100,
                intArrayOf(colorPro1, colorPro2), null, Shader.TileMode.CLAMP
            )
        }
        invalidate()
    }
